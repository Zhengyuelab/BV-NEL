# Instrument communication protocol

This document summarizes the communication protocol used by BV-NEL to communicate with the portable electrochemical workstation. The detailed original protocol is preserved in `docs/protocol_original_v1.md`.


## Instrument information

The BV-NEL implementation described in this repository was tested with a custom portable electrochemical workstation provided by **Wuxi Signal Technology Co., Ltd.** The communication details below document the command frames and returned data format used by this customized instrument and the mobile-side BV-NEL workflow.
...
In the current app workflow, OCP is read for 40 s with 0.1 s polling interval, and the final 1 s is averaged to obtain E<sub><i>initial</i></sub>.
